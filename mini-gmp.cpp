...
#define GMP_LIMB_HIGHEBIT ((mp_limb_t) 1 << (GMP_LIMB_BITS - 1))

#define GMP_HLIMB_BIT ((mp_limb_t) 1 << (GMP_LIMB_BITS / 2))
#define GMP_LLIMB_MASK (GMP_HLIMB_BIT - 1)

#define GMP_ULONG_BITS (sizeof(unsigned long) * CHAR_BIT)
#define GMP_ULONG_HIGHEBIT ((unsigned long) 1 << (GMP_ULONG_BITS - 1))
...
#define gmp_clz(count, x) do {						\
    ...
    for (__clz_c = 0;							\
	 ...
	 __clz_c += 8)							\
      __clz_x <<= 8;							\
    for (; (__clz_x & GMP_LIMB_HIGHEBIT) == 0; __clz_c++)		\
      __clz_x <<= 1;							\
    (count) = __clz_c;							\
  } while (0)
		   ...
		   const struct gmp_div_inverse *inv)
{
  ...
  if (dn == 1)
  ...
  else
    {
      ...
      assert ((inv->d1 & GMP_LIMB_HIGHEBIT) != 0);
...
mpz_fits_slong_p (const mpz_t u)
{
  ...
  if (us == 1)
    return u->_mp_d[0] < GMP_LIMB_HIGHEBIT;
  else if (us == -1)
    return u->_mp_d[0] <= GMP_LIMB_HIGHEBIT;
  else
    return (us == 0);
...
mpz_get_si (const mpz_t u)
{
  ...
  if (us > 0)
    return (long) (u->_mp_d[0] & ~GMP_LIMB_HIGHEBIT);
  else if (us < 0)
    return (long) (- u->_mp_d[0] | GMP_LIMB_HIGHEBIT);
  else
    return 0;
...
mpz_set_d (mpz_t r, double x)
{
  ...
  B = 2.0 * (double) GMP_LIMB_HIGHEBIT;
  Bi = 1.0 / B;
  for (rn = 1; x >= B; rn++)
    x *= Bi;
...
mpz_get_d (const mpz_t u)
{
  mp_size_t un;
  double x;
  double B = 2.0 * (double) GMP_LIMB_HIGHEBIT;
